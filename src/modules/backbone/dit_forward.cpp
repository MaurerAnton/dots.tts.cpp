    ...
    ggml_context * ctx, ggml_tensor * t_tensor, dit_model & model)
{
    ...
    { float * w1 = tensor_data(model.t_embed_w1); float * b1 = model.t_embed_b1 ? tensor_data(model.t_embed_b1) : nullptr;
      for (int o = 0; o < 1024; o++) { float s = b1 ? b1[o] : 0.0f;
          for (int i = 0; i < emb_dim; i++) s += w1[o * 256 + i] * sin_emb[i]; h1[o] = s / (1.0f + expf(-s)); } }
    float h2[1024];
    { float * w2 = tensor_data(model.t_embed_w2); float * b2 = model.t_embed_b2 ? tensor_data(model.t_embed_b2) : nullptr;
      for (int o = 0; o < 1024; o++) { float s = b2 ? b2[o] : 0.0f;
          for (int i = 0; i < 1024; i++) s += w2[o * 1024 + i] * h1[i]; h2[o] = s; } }
...
static void compute_speaker_manual(dit_model & model, ggml_tensor * speaker_emb, float * out) {
    ...
    float * se = tensor_data(speaker_emb); float temp[1024];
    for (int o = 0; o < 1024; o++) { float s = sb1 ? sb1[o] : 0.0f; for (int i = 0; i < 512; i++) s += sw1[o * 512 + i] * se[i]; temp[o] = s; }
    float mean = 0, var = 0; for (int i = 0; i < 1024; i++) mean += temp[i]; mean /= 1024;
    for (int i = 0; i < 1024; i++) { float d = temp[i] - mean; var += d * d; } var = var / 1024 + 1e-5f;
...
ggml_tensor * dit_timestep_embedding(ggml_context * ctx, ggml_tensor * t, int dim) {
    ...
    for (int b = 0; b < n_batch; b++) { float t_val = td[b];
        for (int i = 0; i < half_dim; i++) { float freq = expf(-logf(10000.0f) * (float)i / (float)half_dim);
            float arg = t_val * freq; ed[b * dim + i] = cosf(arg); ed[b * dim + half_dim + i] = sinf(arg); } }
    return emb;
}

static ggml_tensor * dit_ffn(ggml_context * ctx, ggml_tensor * x,
    ggml_tensor * w1, ggml_tensor * w2, ggml_tensor * b1, ggml_tensor * b2)
{ ggml_tensor * h = ggml_mul_mat(ctx, w1, x); if (b1) h = ggml_add(ctx, h, b1);
  h = ggml_silu(ctx, h); h = ggml_mul_mat(ctx, w2, h); if (b2) h = ggml_add(ctx, h, b2); return h; }
    ...
    ggml_tensor * x, ggml_tensor * cond, dit_block & block, int seq_len, int n_batch)
{
    ...
    { float * aw = tensor_data(block.adaln_linear_w); float * ab = block.adaln_linear_b ? tensor_data(block.adaln_linear_b) : nullptr;
      for (int o = 0; o < 6 * hidden; o++) { float s = ab ? ab[o] : 0.0f;
          for (int i = 0; i < DIT_HIDDEN_SIZE; i++) s += aw[o * DIT_HIDDEN_SIZE + i] * cond_silu[i]; adaln_raw[o] = s; } }
    ...
    auto broadcast = [&](ggml_tensor * mod) { ggml_tensor * out = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, hidden, n_tokens);
        float * src = tensor_data(mod), * dst = tensor_data(out);
        for (int s = 0; s < seq_len; s++) for (int b = 0; b < n_batch; b++)
            memcpy(dst + (b * seq_len + s) * hidden, src + b * hidden, hidden * sizeof(float)); return out; };
    ...
    h = ggml_mul(ctx, ggml_add(ctx, ones, scm_t), h); h = ggml_add(ctx, h, sm_t);
    ggml_tensor * attn = dit_attention_multihead(ctx, h,
        ...
        nullptr, nullptr, nullptr, block.attn_o_bias,
        seq_len, n_batch, DIT_NUM_HEADS, DIT_HEAD_SIZE, block.q_norm_w, block.k_norm_w);
    x = ggml_add(ctx, x, ggml_mul(ctx, gm_t, attn));

    h = ggml_rms_norm(ctx, x, 1e-6f); if (block.ffn_norm_w) h = ggml_mul(ctx, h, block.ffn_norm_w);
    h = ggml_mul(ctx, ggml_add(ctx, ones, scl_t), h); h = ggml_add(ctx, h, sml_t);
    ggml_tensor * ffn = dit_ffn(ctx, h, block.ffn_w1, block.ffn_w2, block.ffn_b1, block.ffn_b2);
    x = ggml_add(ctx, x, ggml_mul(ctx, gml_t, ffn));

    // DEBUG: dump block 0 output
    static int blk_cnt = 0;
    if (blk_cnt == 0) { float * xd = tensor_data(x); float r = 0; int nf = 0;
        for (int i = 0; i < hidden * n_tokens; i++) if (std::isfinite(xd[i])) { r += xd[i]*xd[i]; nf++; }
        fprintf(stderr, "  block0_out: rms=%.4f finite=%d first3=[%.4f,%.4f,%.4f]\n", nf>0?sqrtf(r/nf):0.0f, nf, xd[0], xd[1], xd[2]); }
    blk_cnt++;
    return x;
}
...
{
    int seq_len = x->ne[0], n_batch = x->ne[1], hidden = DIT_HIDDEN_SIZE, n_tokens = seq_len * n_batch;
    x = ggml_cont(ctx, ggml_permute(ctx, x, 2, 0, 1, 3)); x = ggml_reshape_2d(ctx, x, hidden, n_tokens);
    ggml_tensor * t_emb = compute_time_embedding_manual(ctx, t, model);
    ggml_tensor * cond;
    if (speaker_emb && model.spk_proj_w1) { float spk_vals[1024]; compute_speaker_manual(model, speaker_emb, spk_vals);
        float * te = tensor_data(t_emb); cond = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, DIT_HIDDEN_SIZE, n_batch);
        float * cd = tensor_data(cond); for (int i = 0; i < 1024; i++) cd[i] = te[i] + spk_vals[i]; }
    else cond = t_emb;
    if (model.input_layer_w) { x = ggml_mul_mat(ctx, model.input_layer_w, x); if (model.input_layer_b) x = ggml_add(ctx, x, model.input_layer_b); }
    // DEBUG verify input_layer data after compute (will be correct since x is graph node)
    for (int i = 0; i < model.n_layers; i++) x = dit_block_forward_simple(ctx, x, cond, model.layers[i], seq_len, n_batch);
    ggml_tensor * out = x;
    if (model.out_adaln_w) { float cs[1024]; { float * cd = tensor_data(cond); for (int i = 0; i < 1024; i++) cs[i] = cd[i] / (1.0f + expf(-cd[i])); }
        float mr[2*DIT_HIDDEN_SIZE]; { float * aw = tensor_data(model.out_adaln_w); float * ab = model.out_adaln_b ? tensor_data(model.out_adaln_b) : nullptr;
          for (int o = 0; o < 2*hidden; o++) { float s = ab ? ab[o] : 0.0f; for (int i = 0; i < DIT_HIDDEN_SIZE; i++) s += aw[o*DIT_HIDDEN_SIZE+i]*cs[i]; mr[o]=s; } }
        ggml_tensor * mod = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 2*DIT_HIDDEN_SIZE, 1); memcpy(tensor_data(mod), mr, 2*DIT_HIDDEN_SIZE*sizeof(float));
        int str = DIT_HIDDEN_SIZE;
        ggml_tensor * sh = ggml_view_2d(ctx, mod, DIT_HIDDEN_SIZE, n_batch, mod->nb[1], 0);
        ggml_tensor * sc = ggml_view_2d(ctx, mod, DIT_HIDDEN_SIZE, n_batch, mod->nb[1], str*sizeof(float));
        ...
        { float *ss = tensor_data(sh), *sd = tensor_data(st); float *cs2 = tensor_data(sc), *cd = tensor_data(sct);
          for (int s = 0; s < seq_len; s++) for (int b = 0; b < n_batch; b++) {
              memcpy(sd+(b*seq_len+s)*DIT_HIDDEN_SIZE, ss+b*DIT_HIDDEN_SIZE, DIT_HIDDEN_SIZE*sizeof(float));
              memcpy(cd+(b*seq_len+s)*DIT_HIDDEN_SIZE, cs2+b*DIT_HIDDEN_SIZE, DIT_HIDDEN_SIZE*sizeof(float)); } }
        ...
        out = ggml_mul(ctx, out, ggml_add(ctx, ones, sct)); out = ggml_add(ctx, out, st); }
    out = ggml_mul_mat(ctx, model.out_proj_w, out); if (model.out_proj_b) out = ggml_add(ctx, out, model.out_proj_b);
    return out;
}
