// Test: Use Python's hidden_proj to drive C++ DiT + BigVGAN
// This isolates DiT+Vocoder correctness from LM differences
#include "dots_tts.h"
...
static void compute_cond(dit_model & m, float t_val, const float * spk_emb, float * cond) {
    ...
    if(spk_emb){float t[1024],*sw1=tensor_data(m.spk_proj_w1),*sb1=m.spk_proj_b1?tensor_data(m.spk_proj_b1):nullptr;
     ...
     float*lnw=m.spk_ln_w?tensor_data(m.spk_ln_w):nullptr;float*lnb=m.spk_ln_b?tensor_data(m.spk_ln_b):nullptr;
     float mn=0;for(int i=0;i<1024;i++)mn+=t[i];mn/=1024;
     float vr=0;for(int i=0;i<1024;i++){float d=t[i]-mn;vr+=d*d;} float is=1.0f/sqrtf(vr/1024+1e-5f);
     for(int i=0;i<1024;i++){float v=(t[i]-mn)*is;if(lnw)v*=lnw[i];if(lnb)v+=lnb[i];cond[i]+=v;}}
}
...
int main() {
    ...
    if (!bigvgan_load("models/vocoder_eff.safetensors", bv)) {
        fprintf(stderr, "FAILED BigVGAN\n"); return 1;
    }
    
    // Count Python dump files
    int n_calls = 0;
    float *all_latents = nullptr;
    int total_frames = 0;
    const int latent_dim = 128, patch_size = 4, nfe = 10;
    const float dt = 1.0f / nfe;
    
    // First pass: count calls and collect all latents
    for (int call = 0; ; call++) {
        char fname[256];
        snprintf(fname, sizeof(fname), "py_call%d_latents.bin", call);
        FILE * f = fopen(fname, "rb");
        if (!f) break;
        fseek(f, 0, SEEK_END);
        long sz = ftell(f);
        fclose(f);
        int frames = sz / (latent_dim * sizeof(float));
        total_frames += frames;
        n_calls = call + 1;
    }
    
    if (n_calls == 0) { fprintf(stderr, "No Python dump files found\n"); return 1; }
    fprintf(stderr, "Found %d calls, %d total frames\n", n_calls, total_frames);
    
    all_latents = new float[total_frames * latent_dim];
    int frame_offset = 0;
    
    // Second pass: run DiT flow matching
    for (int call = 0; call < n_calls; call++) {
        fprintf(stderr, "Call %d/%d: ", call+1, n_calls);
        
        // Load Python fm_sequence for this call
        char fname[256];
        ...
        fread(fm_seq, sizeof(float), fm_seq_len * DIT_HIDDEN_SIZE, f);
        fclose(f);
        ...
        for (int step = 0; step < nfe; step++) {
            ...
            // Build DiT input: fm_sequence + coord_proj(noise)
            ...
            for (int p = 0; p < cond_seq; p++) {
                float *pos = dx + p * DIT_HIDDEN_SIZE;
                float r = 0; for (int j = 0; j < DIT_HIDDEN_SIZE; j++) r += pos[j]*pos[j];
                r = sqrtf(r/DIT_HIDDEN_SIZE);
                if (r > 10.0f) { float s = 10.0f/r; for(int j=0;j<DIT_HIDDEN_SIZE;j++) pos[j]*=s; }
            }
            
            // Run DiT
            float *out_data = new float[cond_seq * latent_dim];
            float cond[1024]; compute_cond(dit, t, nullptr, cond);
            float *h = new float[cond_seq * DIT_HIDDEN_SIZE];
            ...
            float *bo = new float[cond_seq * DIT_HIDDEN_SIZE];
            ...
            for (int p = 0; p < patch_size; p++) {
                ...
                for (int c = 0; c < latent_dim; c++)
                    v_t[p * latent_dim + c] = out_data[t_idx * latent_dim + c];
            }
        ...
        // Store latents
        memcpy(all_latents + frame_offset * latent_dim, z_t, patch_size * latent_dim * sizeof(float));
        frame_offset += patch_size;
        
        { float r=0; for(int i=0;i<patch_flat;i++) r+=z_t[i]*z_t[i];
          fprintf(stderr, "latent RMS=%.4f\n", sqrtf(r/patch_flat)); }
    ...
    // Write WAV
    FILE * wf = fopen("output_pyctx.wav", "wb");
    if (wf) {
        ...
        fclose(wf);
        fprintf(stderr,"Written output_pyctx.wav: %d samples\n", n_samples);
    }
    
    delete[] all_latents; delete[] audio;
    bigvgan_free(bv);
    ggml_free(w_ctx);
    return 0;
}
